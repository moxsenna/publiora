...
const dim = {
  sm: { box: "h-7 w-7", text: "text-base", glyph: 14 },
  md: { box: "h-9 w-9", text: "text-lg", glyph: 18 },
  lg: { box: "h-12 w-12", text: "text-2xl", glyph: 24 },
};
...
export function Logo({ size = "md", href = "/", showText = true, className }: LogoProps) {
  ...
  const content = (
    <div className={cn("inline-flex items-center gap-2", className)}>
      <div
        className={cn(
          "rounded-[10px] bg-[var(--color-publiora-black)] grid place-items-center text-[var(--color-publiora-white)]",
          d.box
        )}
      >
